...
function SelectOption({ label, buttonText, data, onChange, id, selectedId,disabled=false,  RightIcon, LeftIcon, props, RightBox, space, }) {
    return (
        <div style={props?.style} className={`${styles.container} ${props?.additionalcontainer}`}>
            <label className={styles.inputtxt}>{label}</label>
            {space && <dev style={{ marginTop: 16 }}></dev>}
            <div  >
                <select className={`${styles.content} ${props?.additionalcontent}`} id={id} value={selectedId} onChange={onChange} disabled={disabled}  >
